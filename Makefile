...
bench-build/$(UPTODATE): bench-build/*
...
$(EXES) $(PROTO_GOS) lint test shell: bench-build/$(UPTODATE)
	@mkdir -p $(shell pwd)/.pkg
	$(SUDO) docker run $(RM) -ti \
		-v $(shell pwd)/.pkg:/go/pkg \
		-v $(shell pwd):/go/src/github.com/weaveworks-experiments/prometheus-benchmarks \
		$(IMAGE_PREFIX)bench-build $@

else

$(EXES): bench-build/$(UPTODATE)
	go build $(GO_FLAGS) -o $@ ./$(@D)
	$(NETGO_CHECK)

%.pb.go: bench-build/$(UPTODATE)
	protoc -I ./vendor:./$(@D) --gogoslick_out=plugins=grpc:./$(@D) ./$(patsubst %.pb.go,%.proto,$@)

lint: bench-build/$(UPTODATE)
	./tools/lint -notestpackage -ignorespelling queriers -ignorespelling Queriers .

test: bench-build/$(UPTODATE)
	./tools/test -netgo

shell: bench-build/$(UPTODATE)
	bash
